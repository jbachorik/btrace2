...
//    private volatile List<WeakReference<Class<?>>> classes = new ArrayList<WeakReference<Class<?>>>();
    ...
    protected final boolean debug = BTraceLogger.isDebug();
    ...
    static {
        ...
        BTraceRuntimeBridge.class.getClass();
        ...
        try {
            if (isBTraceClass(cname) || isSensitiveClass(cname)) {
                ...
                if (classBeingRedefined != null) {
                    ...
                    BTraceLogger.debugPrint("extension resource injection prepared");
                    if (bytecode == null) {
...
//        classes.add(new WeakReference<Class<?>>(classBeingRedefined));
    ...
    protected synchronized void onExit(int exitCode) {
        if (shouldAddTransformer()) {
            BTraceLogger.debugPrint("onExit: removing transformer for " + className); // NOI18N
...
//            }
        ...
        try {
            BTraceLogger.debugPrint("onExit: closing all"); // NOI18N
            Thread.sleep(300);
            closeAll();
...
//        this.runtime = new BTraceRuntime(className, args, this, inst, repository);
        ...
        BTraceLogger.debugPrint("sending Okay command"); // NOI18N
        // FIXME
...
//        runtime.send(new RetransformationStartNotification(numClasses));
        if (MainOld.isDebug()) BTraceLogger.debugPrint("calling retransformClasses (" + numClasses + " classes to be retransformed)"); // NOI18N
    }
