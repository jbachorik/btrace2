...
class RemoteClient extends Client {
    ...
    RemoteClient(Instrumentation inst, Socket sock, ExtensionsRepository eLocator) throws IOException {
        super(inst, eLocator);
        this.sock = sock;
        this.ois = new ObjectInputStream(sock.getInputStream());
        this.oos = new ObjectOutputStream(sock.getOutputStream());
        // FIXME
//        Command cmd = Command.readCommand(ois);
...
//                                getRuntime().exit(ecmd.getExitCode());
//                            } catch (Throwable th) {
