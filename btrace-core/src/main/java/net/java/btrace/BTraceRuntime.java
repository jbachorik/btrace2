...
public final class BTraceRuntime {
    ...
    private volatile List<GarbageCollectorMXBean> gcBeanList;
...
//        dataLinkThread = null;
     ...
     */
    public BTraceRuntime(final String runtimeName, String[] args, Channel commChannel,
...
//        this.dataLinkThread = new Thread(new Runnable() {
//
//            public void run() {
//                while(!Thread.currentThread().isInterrupted()) {
//                    try {
//                        AbstractCommand cmd = queue.take();
...
//                queue.clear();
//            }
//        }, "BTrace Server DataLink");
//        this.dataLinkThread.setDaemon(true);
//        this.dataLinkThread.start();
    }
    ...
    public static String getValidTraceClassName(String className) {
        int cntr = 1;
        
        while (true) {
            try {
                ClassLoader.getSystemClassLoader().loadClass(className);
                className = className + "$" + cntr;
    ...
    public static <T extends AbstractCommand> void send(Class<? extends T> cmdClass, AbstractCommand.Initializer<T> init) {
        send(cmdClass, init, getCurrent());
    }
    
    private static <T extends AbstractCommand> void send(Class<? extends T> cmdClass, AbstractCommand.Initializer<T> init, BTraceRuntime rt) {
        try {
            rt.channel.sendCommand(cmdClass, init);
