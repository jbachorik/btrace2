...
@BTrace class Profiling {
    @Property
    Profiler swingProfiler = newProfiler();
    
    @OnMethod(clazz="/javax\\.swing\\..*/", method="/.*/")
    void entry(@ProbeMethodName(fqn=true) String probeMethod) { 
        recordEntry(swingProfiler, probeMethod);
    }
    
    @OnMethod(clazz="/javax\\.swing\\..*/", method="/.*/", location=@Location(value=Kind.RETURN))
    void exit(@ProbeMethodName(fqn=true) String probeMethod, @Duration long duration) { 
        recordExit(swingProfiler, probeMethod, duration);
    }
    
    @OnTimer(5000)
    void timer() {
        printSnapshot("Swing performance profile", snapshot(swingProfiler));
    }
}
